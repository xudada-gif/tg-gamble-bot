...
async def bet(update: Update, context: CallbackContext):
    if not context.bot_data["running"]:
    ...
    try:
        ...
        # 查询用户余额
        user_info = get_user_info_db(cursor, user_id)
        if user_info[0]['bet_choice'] in ["大", "小"]:
            await update.message.reply_text("❌ 你本轮已经存在押注！")
            return
        if not user_info:
            await update.message.reply_text("❌ 您还未进行任何充值，请使用 /start 加入！")
            return
        if user_info[0]["money"] < amount:
            await update.message.reply_text(f"❌ 余额不足，你当前只有 {user_info[0]['money']} 金币！ 请添加机器人充值！")
            return
    ...
    context.bot_data["bet_users"][update.message.from_user.id] = (amount, choice)
    print(context.bot_data["bet_users"])
...
async def cancel_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if res[0]["bet_amount"] < 0:
        ...
        return
    # 2、情况押注大小，返回押注金额
    update_balance_db(conn, cursor, user_id, res[0]["bet_amount"])
...
async def show_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_info[0]["bet_amount"] == 0:
        await update.message.reply_text("❌ 你还未押注！")
        return

    await update.message.reply_text(f"🎲 你押注了 {user_info[0]['bet_amount']} 金币，方向：{user_info[0]['bet_choice']}")
    conn.close()
